...
def _naive_test_coverage(repo_path, all_affected, changed_entities):
    ...
    if not tests_dir.exists(): return set()
    corpus="\n".join(p.read_text(errors="replace") for p in tests_dir.glob("test_*.py"))
    covered=set()
    for entity_id in list(all_affected)+list(changed_entities):
        bits=entity_id.split("."); leaf=bits[-1]; cls=bits[-2] if len(bits)>1 else ""
        if cls and cls.lower() in corpus.lower() and leaf+"(" in corpus: covered.add(entity_id)
    return covered

def _test_recommendations(tests, affected, changed_entities, covered):
    affected_names={a["entity_id"] for a in affected}; tokens=set()
    ...
    for t in tests:
        ...
        # Explicit name/description matches outrank generic tests.
        score=min(100, overlap*22 + (35 if any(e.split(".")[-1].lower() in hay for e in affected_names) else 0))
        if score==0:
            # A test can still be relevant if its name shares a changed module token.
            score=min(100, sum(8 for x in tokens if x in hay))
        priority="critical" if score>=70 else "recommended" if score>=25 else "optional"
        if score>0: rec.append({"test_id":t["test_id"],"priority":priority,"relevance_score":round(score,1),"reason":f"matched {overlap} impact-context token(s)"})
...
def _missed_risks(affected, covered, historical_matches):
    ...
    for a in affected:
        ...
        out.append({"entity_id":eid,"missed_risk_score":score,"reason":("historical evidence also references this path" if evidence else "no direct test coverage found for this affected entity"),"supporting_checkpoint_id":historical_matches[0].get("commit_id") if evidence and historical_matches else None})
    return sorted(out,key=lambda x:-x["missed_risk_score"])[:8]

def _mlflow_trace(context, fn):
    if os.getenv("MLFLOW_ENABLED","false").lower() not in {"1","true","yes","on"}: return fn()
    try:
        import mlflow
        if hasattr(mlflow,"trace"):
            @mlflow.trace(name="impactlens.analysis", span_type="AGENT")
            def traced(): return fn()
            return traced()
    except Exception as exc: logger.warning("MLflow tracing disabled: %s",exc)
    return fn()

def run_analysis(repo_path, repo_name, commit, db_path):
    if not commit_exists(repo_path,commit): raise AnalysisError(f"'{commit}' is not a valid commit/ref in this repository")
    try: result=db_pipeline.run_pipeline(repo_path,repo_name,commit,db_path)
    except GitError as exc: raise AnalysisError(str(exc)) from exc
    conn=db_pipeline.get_connection(db_path); tests=db_pipeline.fetch_tests(conn,repo_name)
    ...
    if databricks_enabled():
        try: historical=AISearchStore().search(search_query,k=8,repo=repo_name)
        except Exception as exc: logger.warning("Databricks AI Search failed: %s",exc); historical=[]
    else: historical=CheckpointIndex(conn,repo_name).search(search_query,k=8,historical_only=True)
    ...
    conn.execute("DELETE FROM gold_risk_scores WHERE commit_id=?",(commit,)); conn.execute("INSERT OR REPLACE INTO gold_risk_scores VALUES (?,?,?,?,?)",(repo_name,commit,risk["risk_score"],risk["risk_band"],__import__("json").dumps(risk["risk_reasons"]))); conn.commit()
    context={"repo":repo_name,"commit":result["commit_meta"],"changed_files":result["changed_files"],"changed_entities":result["changed_entities"],"affected_entities":affected,"developer_checkpoint":result["checkpoint"],"historical_checkpoints_matched":historical,"risk":risk,"recommended_tests":recommended,"candidate_missed_risks":missed}
    report=_mlflow_trace(context,lambda: llm_analyze(context))
    run_id=str(uuid.uuid4())
    final={"repo":repo_name,"commit":result["commit_meta"],"changed_files":result["changed_files"],"changed_entities":result["changed_entities"],"affected_entities":affected,"risk":risk,"developer_checkpoint":result["checkpoint"],"historical_checkpoints_matched":historical,"recommended_tests":recommended,"candidate_missed_risks":missed,"report":report,"data_plane":"databricks" if databricks_enabled() else "local-demo","run_id":run_id}
    if databricks_enabled():
        try:
            DatabricksStore().write(result,risk,tests=tests,recommended_tests=recommended,missed=missed,run_id=run_id,llm_engine=report.get("engine","unknown")); final["databricks_write"]="ok"
        except Exception as exc: final["databricks_write"]=f"error: {exc}"
    return final
